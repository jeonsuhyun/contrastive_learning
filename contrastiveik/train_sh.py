...
def train_epoch(model_transformer, data_loader, optimizer, device, epoch):
    ...
    for step, (x_i, x_null, label, c) in enumerate(data_loader):
        ...
        loss = criterion_hmlc(z_concat,label)
...
if __name__ == "__main__":
    ...
    model_transformer = network.TransformerNetwork(
        input_dim=args.input_dim,
        feature_dim=args.feature_dim,
        instance_dim=args.instance_dim,
        cluster_dim=args.cluster_dim
    ...
    if args.reload:
        ...
        model_transformer.load_state_dict(ckpt['net'])
    ...
    criterion_hmlc = contrastive_loss.HMLC(
        temperature=0.07,
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        if epoch % 100 == 0 or epoch == args.epochs:
            save_checkpoint(model_transformer, optimizer, epoch, args)
