...
class VoxelValidityNetModule(pl.LightningModule):
    def __init__(self, h_dim, z_dim, x_dim=None, c_dim=0, scene_range=range(500), n_grid=32, max_config_len=1000, tag_name='no_tag',
        ...
        """dataset with device"""
        train_dataset = TensorDataset(D0[:train_set_len],#.to(device), 
                                      Y0[:train_set_len],#.to(device),
                                      C0[:train_set_len],#.to(device),
                                       V0[:train_set_len])
        validation_dataset = TensorDataset(D0[train_set_len:],#.to(device), 
                                           Y0[train_set_len:],#.to(device),
                                           C0[train_set_len:],#.to(device),
                                           V0[train_set_len:])
